...
def split_epub_to_chapter_pdfs(epub_path, output_dir=None):
    """
    Main function to split EPUB into chapter PDFs
    ...
    print(f"[*] Splitting {epub_path.name} into individual chapter PDFs...")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for chapter_info in chapters:
            print(f"[*] Processing Chapter {chapter_info['number']}: {chapter_info['title']}")
            ...
            try:
                ...
                # Convert to PDF
                pdf_filename = f"chapter_{chapter_info['number']:02d}_{chapter_info['clean_title']}.pdf"
                ...
                print(f"[✗] Error processing chapter {chapter_info['number']}: {e}")
        
        print(f"\n[✓] Conversion complete!")
        print(f"[*] Successfully converted {successful_conversions}/{len(chapters)} chapters")
